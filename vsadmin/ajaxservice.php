...
<?php
	...
	if($lastapplied < 3 && @$_GET['act']!='delete'){
		...
		$gotcpncode = FALSE;
		if(getget('stg1')!='1') print '<table border="0"><tr><td align="center" colspan="3">';
		if(@$_GET['act']=='delete'){
			$gotcpncode = TRUE;
			...
			if($rs = ect_fetch_assoc($result)){
				if(strpos(@$_SESSION['giftcerts'], $rs['gcID'] . ' ')===FALSE){
					...
					print $GLOBALS['xxGcApld'];
				}else
					print $GLOBALS['xxGcAlAp'];
				$gotcpncode = TRUE;
			}
			ect_free_result($result);
		}
		...
		if(getget('stg1')!='1') print '</td></tr>';
		if((@$_SESSION['giftcerts']!='' || @$_SESSION['cpncode']!='') && getget('stg1')!='1'){
			$gcarr = explode(' ', trim(@$_SESSION['giftcerts']));
			foreach($gcarr as $key => $value){
				if($value!='') print '<tr><td align="right">' . $GLOBALS['xxAppGC'] . ':</td><td><div style="float:left;padding:2px">' . $value . '</div><div style="float:left;padding:2px">(<a href="#" onclick="return removecert(\''.$value.'\')"><strong>'.$GLOBALS['xxRemove'].'</strong></a>)</div></td></tr>';
			}
			$cpnarr = explode(' ', trim(@$_SESSION['cpncode']));
			foreach($cpnarr as $key => $value){
				if($value!='') print '<tr><td align="right">' . $GLOBALS['xxApdCpn'] . ':</td><td><div style="float:left;padding:2px">' . $value . '</div><div style="float:left;padding:2px">(<a href="#" onclick="return removecert(\''.$value.'\')"><strong>'.$GLOBALS['xxRemove'].'</strong></a>)</div></td></tr>';
			}
		}
		if(getget('stg1')!='1') print '</table>';
	}
			...
			if($csvline!=''){
				if($columnarr=explode($fileseparator,$csvline)){
					...
					if(($isdazzlefile && $theaddress!='') || ($isworldshipfile && $theemail!='')){
						...
						if($addressarr=explode(',',$theaddress) || $isworldshipfile){
