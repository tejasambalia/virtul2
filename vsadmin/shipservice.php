		...
		if(substr($key,0,6)=='prodid'){
			...
			if(is_numeric($prodindex)&&is_numeric(@$_GET['quant' . $prodindex])){
				...
				while($rs=ect_fetch_assoc($result)){
					...
					$alldataarr[$itemsincart]['cartProdPrice']+=$optpricediff;
					$alldataarr[$itemsincart]['pWeight']+=$optweightdiff;
					$itemsincart++;
				}
				ect_free_result($result);
			}
...
if($itemsincart>0){
	for($indexserv=0; $indexserv<$itemsincart; $indexserv++){
		...
		if(is_numeric($alldata['cartID'])){
			...
			addproducttoshipping($alldata, $indexserv+1);
		}
	}
}else{
...
if($success && calculateshipping()){
	if(getget('ratetype')=='estimator'){
		...
		if($isadmincalc) $orderid=0; else $orderid=getget('orderid');
		if(is_numeric($orderid)){
			...
			if($shipType>=2){
				if(@$shippingoptionsasradios!=TRUE) print '<select size="1" onchange="updateshiprate(this,(this.selectedIndex-1)+'.$numshiprate.')"><option value="">'.$GLOBALS['xxPlsSel'].' ('.$GLOBALS['xxFromSE'].': '.FormatEuroCurrency(($shipping+(@$combineshippinghandling?$handling:0))-$freeshipamnt).')</option>'; else print '<table border="0">';
