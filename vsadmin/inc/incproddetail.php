...
function displaytabs($thedesc){
	...
	if($hasdesctab || @$ecttabsspecials!='' || @$ecttabs!='' || @$defaultdescriptiontab!=''){
		if(@$defaultdescriptiontab=='')$defaultdescriptiontab='<ecttab title="'.$GLOBALS['xxDescr'].'">';
...
if(@$GLOBALS['magictoolbox']!=''){ $GLOBALS['magictool']=$GLOBALS['magictoolbox']; $GLOBALS['giantimageinpopup']=FALSE; }
...
function writepreviousnextlinks(){
	global $previousid,$previousidname,$previousidstatic,$previousstaticurl,$previousidcat,$nextid,$nextidname,$nextidstatic,$nextstaticurl,$nextidcat,$thecatid,$catid;
...
function showreviews($theSQL,$showall){
	...
	if($numreviews > 0){
		...
		while($rs2=ect_fetch_assoc($result)){
			...
			$srv.='<br /><br /><span class="review reviewname"><span class="reviewer" itemprop="author">' . $rs2['rtPosterName'] . '</span> - <span class="dtreviewed">' . date($dateformatstr, strtotime($rs2['rtDate'])) . '<meta itemprop="datePublished" content="' . $rs2['rtDate'] . '" /></span></span>';
			...
			$srv.='<br /><br /><span class="summary review reviewcomments" itemprop="description">' . str_replace("\r\n", '<br />', $thecomments) . '</span><br /><hr class="review" />';
			$srv.='</div>';
		}
		ect_free_result($result);
	}else
...
function pddsearchwords(){
	...
	if(trim($rs['pSearchParams'])!=''){
		$searchprms=explode(' ',$rs['pSearchParams']);
...
function pdddescription(){
	...
	if(@$usedetailbodyformat==3){
		...
		print '<div class="detaildescription" itemprop="description">' . displaytabs($longdesc) . '</div>';
...
function pddextracurrency(){
	global $hascustomlayout,$currRate1,$currRate2,$currRate3,$currSymbol1,$currSymbol2,$currSymbol3,$currFormat1,$currFormat2,$currFormat3,$rs,$currencyseparator,$Count,$usecsslayout,$orcdecimals,$orcthousands,$noprice;
...
function pddoptions(){
	...
	if(is_array($prodoptions)){
		...
		if($optionshtml!='') print (@$usecsslayout ? '</div>' : '');
		...
		if(($isinstock || $isbackorder) && @$nobuyorcheckout!=TRUE && (@$showquantondetail==TRUE || ! @isset($showquantondetail))){
			...
			print $GLOBALS['xxQuant'] . ':';
			print (@$usecsslayout ? '</div>' : '</td><td>');
			print '<div class="quantitydiv detailquantityinput" style="'.(@$quantityupdown?'margin-right:12px;':'').'white-space:nowrap"><input type="text" name="quant" id="w'.$Count.'quant" maxlength="5" size="4" value="1" title="' . $GLOBALS['xxQuant'] . '" ' . (@$quantityupdown?'style="float:left" ':'') . '/>' . (@$quantityupdown?'<img src="images/quantarrowu.png" onclick="quantup('.$Count.',1)" alt="" /><br /><img src="images/quantarrowd.png" onclick="quantup('.$Count.',0)" alt="" />':'') . '</div>' . (@$usecsslayout ? '</div>' : '</td></tr></table>');
		}
	}
}
$isfirstaddtocart=TRUE;
function pddaddtocart(){
	global $usecsslayout,$nobuyorcheckout,$rs,$nosellzeroprice,$isinstock,$isbackorder,$wishlistondetail,$imgbackorderbutton,$custombuybutton;
...
function pddemailfriend(){
	...
	if(@$usedetailbodyformat==2 || @$usedetailbodyformat==4){
		...
		print (! @$usecsslayout ? '<tr><td colspan="3" class="detaildescription">' : '') . '<div class="detaildescription" itemprop="description">' . displaytabs($longdesc) . '</div>' . (! @$usecsslayout ? '</td></tr>' : '');
	elseif(trim($rs[getlangid('pDescription',2)])!='')
...
$allgiantimages='';
$numallgiantimages=0;
$sSQL='SELECT pId,pSKU,'.getlangid('pName',1).','.getlangid('pDescription',2).','.$WSP.'pPrice,pSection,pListPrice,pSell,pStockByOpts,pStaticPage,pStaticURL,pInStock,pBackOrder,pExemptions,'.(@$detailslink!='' ? "'' AS " : '').'pTax,pOrder,pDateAdded,pSearchParams,pCustom1,pCustom2,pCustom3,'.(@$manufacturerfield!=''?getlangid('scName',131072).',':'').getlangid('pLongDescription',4).' FROM products '.(@$manufacturerfield!=''?'LEFT OUTER JOIN searchcriteria on products.pManufacturer=searchcriteria.scID ':'').'WHERE pDisplay<>0 AND (' . (@$usepnamefordetaillinks && trim(@$explicitid)==''?getlangid('pName',1):'pId') . "='" . escape_string($prodid) . "'" . (@$seodetailurls?" OR pStaticURL='".escape_string($prodid)."'":'') . ')';
$result=ect_query($sSQL) or ect_error();
...
if((! $productindb && $prodid!=$giftcertificateid && $prodid!=$donationid) || $disabledsection){
	...
	if($prodid!=$giftcertificateid && $prodid!=$donationid){
		...
		if(@$nopreviousnextlinks!=TRUE){
			...
			$result2=ect_query($sSQL) or ect_error();
			if($rs2=ect_fetch_assoc($result2)){
				$nextid=urlencode(@$usepnamefordetaillinks?str_replace(' ',@$detlinkspacechar,$rs2[getlangid('pName',1)]):$rs2['pId']);
			...
			$result2=ect_query($sSQL) or ect_error();
			if($rs2=ect_fetch_assoc($result2)){
				$previousid=urlencode(@$usepnamefordetaillinks?str_replace(' ',@$detlinkspacechar,$rs2[getlangid('pName',1)]):$rs2['pId']);
