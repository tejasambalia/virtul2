					...
					if($e->nodeName[$j]=='Error'){ // Lower-level error
						...
						if($e->nodeName[$j]=='Postage'){
							if($international==''){
								...
								for($index2=0;$index2<$numuspsmeths;$index2++){
									if(str_replace('-',' ',$thisService)==str_replace('-',' ',$uspsmethods[$index2]['uspsMethod'])){ $intShipping[$l][0]=$uspsmethods[$index2]['uspsShowAs']; break; }
								}
							}
							...
							if($international!=''){
								...
								while($intShipping[$l][5]!='' && $intShipping[$l][5]!=$serviceid)
									$l++;
								$intShipping[$l][5]=$serviceid;
								if(!@$GLOBALS['noshipdateestimate']) $intShipping[$l][1]=$SvcCommitments;
...
function addUSPSInternational($id,$iWeight,$quantity,$mailtype,$country,$packcost){
	...
	if(($adminUnits & 12)!=0){
		...
		for($xx=0; $xx < $numuspsmeths; $xx++){
			...
			if($uspsmethods[$xx]['uspsMethod']=='24') if(! doesfitinbox(7.5625,5.4375,0.625)) $uspsmethods[$xx]['uspsMethod']=='xxx'; // DVD FRB
			if($uspsmethods[$xx]['uspsMethod']=='16') if(! doesfitinbox(8.625, 5.375, 1.625)) $uspsmethods[$xx]['uspsMethod']=='xxx'; // Small FRB
			if($uspsmethods[$xx]['uspsMethod']=='20') if(! doesfitinbox(10,6,0.75)) $uspsmethods[$xx]['uspsMethod']=='xxx'; // Small FRE
			if($uspsmethods[$xx]['uspsMethod']=='9' || $uspsmethods[$xx]['uspsMethod']=='26') if(! doesfitinbox(11,8.5,5.5) && ! doesfitinbox(13.625,11.875,3.375)) $uspsmethods[$xx]['uspsMethod']=='xxx'; // FRB
			if($uspsmethods[$xx]['uspsMethod']=='13') if(! doesfitinbox(11.5,6.125,0.25)) $uspsmethods[$xx]['uspsMethod']=='xxx'; // FirstClass Letter
			if($uspsmethods[$xx]['uspsMethod']=='11') if(! doesfitinbox(12,12,5.5)) $uspsmethods[$xx]['uspsMethod']=='xxx'; // LFRB
			if($uspsmethods[$xx]['uspsMethod']=='8' || $uspsmethods[$xx]['uspsMethod']=='10') if(! doesfitinbox(12.5,9.5,1)) $uspsmethods[$xx]['uspsMethod']=='xxx'; // FRE
			if($uspsmethods[$xx]['uspsMethod']=='17') if(! doesfitinbox(15,9.5,0.75)) $uspsmethods[$xx]['uspsMethod']=='xxx'; // Legal FRE
			if($uspsmethods[$xx]['uspsMethod']=='14') if(! doesfitinbox(15,12,0.75)) $uspsmethods[$xx]['uspsMethod']=='xxx'; // FirstClass L-E
			
			if($packdims[0]>7.5625 || $packdims[1]>5.4375 || $packdims[2]>0.625){
				if($uspsmethods[$xx]['uspsMethod']=='24') $uspsmethods[$xx]['uspsMethod']='xxx'; // DVD FRB
			}
			if($packdims[0]>8.625 || $packdims[1]>5.375 || $packdims[2]>1.625){
				if($uspsmethods[$xx]['uspsMethod']=='16') $uspsmethods[$xx]['uspsMethod']='xxx'; // Small FRB
			}
			if($packdims[0]>10 || $packdims[1]>6 || $packdims[2]>0.75){
				if($uspsmethods[$xx]['uspsMethod']=='20') $uspsmethods[$xx]['uspsMethod']='xxx'; // Small FRE
			}
			if(($packdims[0]>11 || $packdims[1]>8.5 || $packdims[2]>5.5) && ($packdims[0]>13.625 || $packdims[1]>11.875 || $packdims[2]>3.375)){
				if($uspsmethods[$xx]['uspsMethod']=='9') $uspsmethods[$xx]['uspsMethod']='xxx'; // FRB
			}
			if($packdims[0]>11.5 || $packdims[1]>6.125 || $packdims[2]>0.25){
				if($uspsmethods[$xx]['uspsMethod']=='13') $uspsmethods[$xx]['uspsMethod']='xxx'; // FirstClass Letter
			}
			if($packdims[0]>12 || $packdims[1]>12 || $packdims[2]>5.5){
				if($uspsmethods[$xx]['uspsMethod']=='11') $uspsmethods[$xx]['uspsMethod']='xxx'; // LFRB
			}
			if($packdims[0]>12.5 || $packdims[1]>9.5 || $packdims[2]>1){
				if($uspsmethods[$xx]['uspsMethod']=='8' || $uspsmethods[$xx]['uspsMethod']=='10') $uspsmethods[$xx]['uspsMethod']='xxx'; // FRE
			}
			if($packdims[0]>15 || $packdims[1]>9.5 || $packdims[2]>0.75){
				if($uspsmethods[$xx]['uspsMethod']=='17') $uspsmethods[$xx]['uspsMethod']='xxx'; // Legal FRE
			}
			if($packdims[0]>15 || $packdims[1]>12 || $packdims[2]>0.75){
				if($uspsmethods[$xx]['uspsMethod']=='14') $uspsmethods[$xx]['uspsMethod']='xxx'; // FirstClass L-E
			}
			if($packdims[0]>24 || ($packdims[0]+$packdims[1]+$packdims[2])>36){
				if($uspsmethods[$xx]['uspsMethod']=='15') $uspsmethods[$xx]['uspsMethod']='xxx'; // FirstClass Package
			}
		}
	}
	$pounds=(int)$iWeight;
