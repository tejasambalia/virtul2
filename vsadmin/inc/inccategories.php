...
if(@$seocategoryurls && @$seourlsthrow301 && @$_SERVER['REDIRECT_URL']==''){
	if($caturl!='')
		...
		$newloc=getfullurl(getcatid($theid,$catname,$seocaturlpattern));
	$addand=$newqs='';
...
if($manufacturers==TRUE){
	$showdiscounts=FALSE;
	$sSQL='SELECT scID AS sectionID,'.getlangid('scName',131072).' AS sectionName,'.getlangid('scDescription',16384).' AS sectionDescription,scLogo AS sectionImage,scOrder AS sectionOrder,1 AS rootSection,'.getlangid('scURL',8192).' AS sectionurl FROM searchcriteria WHERE scGroup=0 ORDER BY ' . (@$sortcategoriesalphabetically==TRUE ? getlangid('scName',131072) : 'scOrder');
}else
...
if($success){
	...
	while($rs=ect_fetch_assoc($result)){
		...
		$sSQL='SELECT DISTINCT '.getlangid('cpnName',1024)." FROM coupons LEFT OUTER JOIN cpnassign ON coupons.cpnID=cpnassign.cpaCpnID WHERE (cpnSitewide=0 OR cpnSitewide=3) AND cpnNumAvail>0 AND cpnEndDate>='" . date('Y-m-d',time()) ."' AND cpnIsCoupon=0 AND cpaType=1 AND cpaAssignment='" . $rs['sectionID'] . "'" .
