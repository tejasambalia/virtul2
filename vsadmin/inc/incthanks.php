...
?>
	<table width="100%" border="0" cellspacing="3" cellpadding="3">
	  <tr> 
		<td width="100%" colspan="2" align="center">&nbsp;<br />&nbsp;<br />&nbsp;<br /><div id="orderfail" style="width:80%"><?php
		print $GLOBALS['xxThkErr'];
		if($errtext!='') print '<p><strong>' . $errtext . '</strong></p>';
		if($paypalwaitipn){
...
?>				<a class="ectlink" href="<?php print $storeurl?>"><strong><?php print $GLOBALS['xxCntShp']?></strong></a><br />&nbsp;
		</div></td>
	  </tr>
	</table>
<?php
...
if(getpost('pprov')=='23'&&getpost('stripeToken')!=''&&getpost('stripeEmail')!=''&&is_numeric(getpost('ordernumber'))){
	...
	if(is_numeric($ordID) && getpayprovdetails(16,$data1,$data2,$data3,$demomode,$ppmethod)){
		...
		order_failed();
}elseif(getget('status')!='' && getget('recipientName')!='' && getget('signature')!=''){ // Amazon Simple Pay
	if(getget('status')=="PF"){
		$errtext="Payment Declined";
		order_failed();
	}elseif(getget('status')=="PS" || getget('status')=="PR" || getget('status')=="PI"){
		$ordID=trim(str_replace("'",'', getget('referenceId')));
		$txn_id=trim(str_replace("'",'', getget('transactionId')));
		if(! is_numeric($ordID)) $ordID=0;
		...
		if($success){
			...
			ect_query("UPDATE cart SET cartCompleted=2 WHERE cartCompleted=0 AND cartOrderID='" . escape_string($ordID) . "'") or ect_error();
			ect_query("UPDATE orders SET ordAuthNumber='no ipn' WHERE ordAuthNumber='' AND ordPayProvider=21 AND ordID='" . escape_string($ordID) . "'") or ect_error();
			$errtext=$GLOBALS['xxNoCnf'];
			$GLOBALS['xxThkErr']='';
			$showclickreload=TRUE;
			order_failed();
