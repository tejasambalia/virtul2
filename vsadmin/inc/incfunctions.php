...
$GLOBALS['redstar']='<span style="color:#FF1010">*</span>';
...
function getadminsettings(){
	...
	if(! @$alreadygotadmin){
		...
		if(@$orcurrencyisosymbol!='') $GLOBALS['countryCurrency']=$orcurrencyisosymbol;
		$GLOBALS['useEuro']=($countryCurrency=='EUR');
		...
		$GLOBALS['adminShipping']=$shipType=(int)$rs['adminShipping'];
		...
		$GLOBALS['sendEmail']=((int)$rs['adminEmailConfirm']==1);
	...
	if(@$orstoreurl!='') $GLOBALS['storeurl']=$orstoreurl;
	...
	if(@$oremailaddr!='') $GLOBALS['allStoreEmails']=$oremailaddr;
	...
	if($origCountryCode=='GB' || $origCountryCode=='IE'){
		...
		if($storelang=='fr' && $origCountryCode=='CA'){
...
function encodeimage($imurl){
	...
	$imurl=str_replace(array('|','<','>','?'),array('%7C','%3C','%3E','%3F'),$imurl);
...
function replaceaccentsutf($surl){
	return(str_replace(array('Ã ','Ã¢','Ã¡','Ã§','Ã¨','Ãª','Ã©','Ã«','Ã®','Ã¯','Ã­','Ã²','Ã´','Ã³','Ã¶','Ã¹','Ã»','Ãº','Ã¼','Ã±','Â®','â„¢'),array('a','a','a','c','e','e','e','e','i','i','i','o','o','o','o','u','u','u','u','n','',''),$surl));
}
...
function ajaxaddcartcb(){
	if(ajaxobj.readyState==4){
		...
		schtml+='<div style="padding:3px;text-align:right" class="scart sccartitems"><?php print $GLOBALS['xxCarCon']?>:'+retvals[4]+' <?php print $GLOBALS['xxSCItem']?></div>' +
		'<div style="padding:3px;text-align:right;background:#f1f1f1;border-top:1px dotted #567CBB;font-weight:bold;color:#666<?php print @$GLOBALS['nopriceanywhere']?';display:none':''?>" class="scart sccarttotal"><span style="color:#8C0000;display:none" id="sccartdscnt" class="scart sccartdscnt">(<?php print $GLOBALS['xxDscnts']?>:<span id="sccartdscamnt" class="sccartdscamnt"></span>) / </span><?php print($GLOBALS['showtaxinclusive']!=0?$GLOBALS['xxCntTax'].':<span id="sccarttax" class="sccarttax"></span> / ':'').$GLOBALS['xxSCCarT']?>:'+pprice+'</div>' +
		'<div style="padding:3px;text-align:right" class="scart sclinks"><a class="ectlink scclink" href="#" onclick="document.getElementById(\'opaquediv\').style.display=\'none\';return false"><?php print $GLOBALS['xxCntShp']?></a> | <a class="ectlink scclink" href="cart.php" onclick="document.getElementById(\'opaquediv\').style.display=\'none\';return true"><?php print $GLOBALS['xxEdiOrd']?></a></div>' +
		'</div>';
		...
		if(document.getElementsByClassName){
			...
			for(var index=0;index<ectMCpm.length;index++)ectMCpm[index].innerHTML='<a href="cart.php"><?php print $GLOBALS['xxClkHere']?></a>';
		...
		if($rs2=ect_fetch_assoc($result)){
			if(abs((int)$rs2['optType'])==3){ // Text
				...
				do {
					...
					$optshtml.='<input type="'.(ect_num_rows($result)==1?'checkbox':'radio').'" data-optgroup="'.$theopt['poOptionGroup'].'" '.($theopt['isDepOpt']==1?'data-isdep="1" ':'').'class="prodoption'.($isdetail?' detailprodoption':'').'" style="vertical-align:middle" onclick="updateprice'.$Count.'();updateoptimage('.$Count.','.$rowcounter.',1)" name="optn'.$Count.'x'.$rowcounter.'" ';
...
function addtomailinglist($theemail,$thename){
	...
	if(strpos($theemail, '@')!==FALSE && strpos($theemail, '.')!==FALSE && strlen($theemail)>5){
		...
		if(! $isconfirmed && ! @$noconfirmationemail && ! $isspam){
			...
			dosendemail($theemail, $emailAddr, '', $GLOBALS['xxMLConf'], $GLOBALS['xxConfEm'] . $emlNl . $emlNl . $thelink);
		}
	}
}
...
function imageorlink($theimg, $thetext, $thelink, $isjs){
	if($theimg!='')
		return '<img style="border:0" src="'.$theimg.'" onmouseover="this.style.cursor=\'pointer\';window.status=\''.str_replace("'","\'",$thetext).'\';return true" onmouseout="window.status=\'\';return true" onclick="'.($isjs ? '' : "document.location=(((ECTbh=document.getElementsByTagName('base')).length>0?ECTbh[0].href+'/':'')+'") . $thelink . ($isjs ? '' : "').replace(/([^:]\/)\/+/g,'$1')").'" alt="'.$thetext.'" />';
	else
...
function getrequest($tval){
	return(unstripslashes(@$_REQUEST[$tval]));
}
...
function getdetailsurl($gdid,$gdstatic,$gdname,$gdurl,$gdqs,$gdpathtohere){
	...
	if(@$seodetailurls)
		$gdgeturl=$gdpathtohere.rawurlencode(str_replace(' ',@$detlinkspacechar,$gdname)).($gdqs!=''?'?'.$gdqs:'');
	elseif($gdstatic!=0)
		$gdgeturl=$gdpathtohere.cleanforurl($gdname).'.php'.($gdqs!=''?'?'.$gdqs:'');
	else
		$gdgeturl=$gdpathtohere.'proddetail.php?prod='.urlencode(@$usepnamefordetaillinks?str_replace(' ',@$detlinkspacechar,$gdname):$gdid).($gdqs!=''?'&amp;'.$gdqs:'');
	return($gdgeturl);
}
