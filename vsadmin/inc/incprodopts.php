		...
		if((getpost('secname')=='' || ! $bOption) && getpost('optType')!='3'){
			...
			if(getpost('optType')=='3'){ // Text option
				...
				if(getpost('act')=='doaddnew'){
					...
					if(getpost('workingname')=="")
						$sSQL.=",'" . escape_string(getpost('secname'));
					else
						$sSQL.=",'" . escape_string(getpost('workingname'));
					$sSQL.="'," . $optFlags . ")";
					ect_query($sSQL) or ect_error();
					$iID =ect_insert_id();
					...
					$sSQL.=",optWeightDiff) VALUES (" . $iID . ",'" . escape_string(getpost('opt0')) . "'," . $fieldDims;
					for($index=2; $index <= $adminlanguages+1; $index++){
						if(($adminlangsettings & 16)==16)
					...
					$sSQL.=",optAcceptChars='" . getpost('optAcceptChars') . "'";
					if(getpost('workingname')=="")
						$sSQL.=",optGrpWorkingName='" . escape_string(getpost('secname')) . "',";
					else
						$sSQL.=",optGrpWorkingName='" . escape_string(getpost('workingname')) . "',";
					$sSQL.="optFlags=" . $optFlags;
					$sSQL.=" WHERE optGrpID=" . $iID;
					ect_query($sSQL) or ect_error();
					$sSQL="UPDATE options SET optName='" . escape_string(getpost('opt0')) . "',optPriceDiff=" . $fieldDims;
					for($index=2; $index <= $adminlanguages+1; $index++){
						if(($adminlangsettings & 16)==16)
							$sSQL.=",optName" . $index . "='" . escape_string(getpost('opl' . $index . 'x0')) . "'";
					}
				...
				if(getpost('act')=='doaddnew'){
					...
					if(getpost('forceselec')=='ON')
						$sSQL.="'".getpost('optType')."',";
					else
						$sSQL.="'".(0-(int)getpost('optType'))."',";
					if(getpost('workingname')=='')
						$sSQL.="'" . escape_string(getpost('secname'));
					else
						$sSQL.="'" . escape_string(getpost('workingname'));
					$sSQL.="'," . $optFlags . ',' . (getpost('optgrpselect')=='1' ? 1 : 0) . ')';
					ect_query($sSQL) or ect_error();
					$iID=ect_insert_id();
				}else{
					$iID=getpost('id');
					...
					if(getpost('forceselec')=="ON")
						$sSQL.=",optType='".getpost('optType')."'";
					else
						$sSQL.=",optType='".(0-(int)getpost('optType'))."'";
					if(getpost('workingname')=="")
						$sSQL.=",optGrpWorkingName='" . escape_string(getpost('secname')) . "',";
					else
						$sSQL.=",optGrpWorkingName='" . escape_string(getpost('workingname')) . "',";
